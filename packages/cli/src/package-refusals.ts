 ...
 *
 * Every door must also run the two-stage contract. The mutation paths (`workspace-package-patch`,
 ...
 *
 * All three verdicts are core's (`validateMotionDocumentInStages`, `unrenderablePackageRefusal`,
 ...
 *
 * Order is public contract: portable structural JSON Schema runs first, then runtime semantic
 * validation. Render-lane and keyframe checks are semantic checks, so a malformed document cannot
 * receive a renderer-specific diagnosis before its portable structure is established.
 *
