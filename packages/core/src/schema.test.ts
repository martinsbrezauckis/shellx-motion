...
describe("schemas", () => {
  it("validates render job handoff contracts for pending/running leased runners", async () => {
  ...
  it("validates prompt job handoff contracts for pending/running local agents", async () => {
  ...
  it("validates single-host platform verification receipts", async () => {
    ...
    const result = await validateDocument(schema, {
      ...
      commandSummary: { total: 2, passed: 1, failed: 0, skipped: 1, skippedByKind: { "capability-absent": 1 } },
      commands: [
  ...
  it("rejects malformed platform verification receipts", async () => {
    ...
    expect(result).toEqual({
      ok: false,
      errors: [
        ...["toolchain", "finishedAt", "commandSummary"].map((field) => ({ path: `/${field}`, message: "required" })),
        { path: "/status", message: "unsupported platform verification status" },
        { path: "/dryRun", message: "must be a boolean" },
        ...["hostname", "platform", "arch", "release", "node"].map((field) => ({ path: `/host/${field}`, message: "required" })),
  ...
  it("rejects unbounded or executable particle emitter values", async () => {
    ...
    expect(await validateDocument(schema, motionWithLayerFields({
      ...
      errors: [
        ...
        { path: "/layers/0/emitter/count", message: "counts above 1000 require the bounded analytic field and a count in 100000..131072" }, { path: "/layers/0/emitter/shape", message: "counts above 1000 require an explicit circle shape for the fixed GPU compute field" },
