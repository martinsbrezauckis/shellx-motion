...
Catalog fingerprint: `8a9d49dd8e5ee964ffa02562e979525461ef5c29f6eac89f75cec6a1055c82ed`. Validate it and every package with `pnpm run docs:rendering-samples:check`.
...
| `family.governed-shader-and-hybrid-gpu-delivery@1` | `sample.tideglass.gpu-h264@1` — `fixtures/packages/gpu-v25b2-tideglass-almanac` | canonical delivery sample | `fixtures/packages/gpu-v25b2-tideglass-almanac/motion.json` | This strict GPU final sample pins one checked restricted-GLSL shader and adjustment-layer source package. It does not claim arbitrary shader code, general hybrid content, a GPU on the current host, or every authoring mutation that can target these layer types. |
...
| `render.gpu-mp4-h264@1` | Strict GPU H.264 MP4 | `fixtures/packages/gpu-v25b2-tideglass-almanac` | `.scratch/rendering-samples/gpu-h264.mp4` (video/mp4) | `render.final` / beside-output |
...
Source checkout:
```bash
pnpm --filter @shellx-motion/cli run cli -- render fixtures/packages/gpu-v25b2-tideglass-almanac --lane ffmpeg --frame-lane gpu --preset mp4-h264 --out .scratch/rendering-samples/gpu-h264.mp4
```

Installed package:
```bash
shellx-motion render <package-root> --lane ffmpeg --frame-lane gpu --preset mp4-h264 --out .scratch/rendering-samples/gpu-h264.mp4
```

Limitation: GPU final is strict raw-RGBA-to-FFmpeg delivery; it requires admitted hardware and refuses unsupported content or fallback lanes.
