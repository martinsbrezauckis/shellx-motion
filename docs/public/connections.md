...
and private access key at call time, so the key is not embedded in the agent configuration. When it
first calls a coordinator job tool, the bridge opens an authenticated persistent WebSocket and keeps
...
above. Coordinator tools open or reuse a persistent connection after that read. Set
