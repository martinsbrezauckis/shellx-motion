...
const RULES = [
  ...
  {
    match: (path) => /^docs\/public\/site\/manual\/motion\/assets\/motion-(about|connections|docs|history|inspector)\.png$/.test(path),
    kind: "Repository-generated Workbench captures",
    origin: "First-party ShellX Motion Workbench screenshots generated by this repository's workbench:ui-smoke browser test, then copied into the online operator manual. They show fixture UI state for interface reference; they are not rendered-media, installed-host, or deployed-site proof.",
    verify: "Run `pnpm run workbench:ui-smoke -- --out <private-scratch-directory>` to reproduce the unprefixed Inspector, History, Connections, Docs, and About captures, compare their SHA-256 values with these motion-prefixed manual copies, then run `python3 build-manual.py --check` from the documented manual source directory to verify the public manual mirror."
  },
  {
